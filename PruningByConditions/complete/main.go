...
func knapsackComplete(weights []int, values []int, capacity int) int {
	n := len(weights)
	items := make([]item, n)
	for i := 0; i < n; i++ {
...
func dfs(items []item, capacity, start, value int, maxValue *int, path []int) {
	...
	for i := start; i < len(items); i++ {
		path = append(path, i)
		dfs(items, capacity-items[i].weight, i, value+items[i].value, maxValue, path)
...
func main() {
	...
	res := knapsackComplete(weights, values, capacity)
